...
def plot_mesh_visualization(
    nodes: np.ndarray, elems: np.ndarray, elem_data: np.ndarray, output_folder: str
) -> None:
...
def plot_electrode_voltages(voltages: np.ndarray, n_elec: int, output_folder: str) -> None:
    ...
    for i, (src, sink) in enumerate(inj_pairs):
        ax1.plot(
            [elec_pos[src, 0], elec_pos[sink, 0]],
    ...
    for i, (src, sink) in enumerate(inj_pairs):
        v_src = (
            voltages[i * (n_elec - 2) * (n_elec - 3) // 2]
            if i * (n_elec - 2) * (n_elec - 3) // 2 < len(voltages)
            else 0
        )
    ...
    ax3.axhline(
        y=voltages.mean(), color="red", linestyle="--", label=f"Mean: {voltages.mean():.2f} V"
    )
    ...
    ax4.axvline(
        x=voltages.mean(), color="red", linestyle="--", label=f"Mean: {voltages.mean():.2f} V"
    )
...
def save_electrode_info(n_elec: int, output_folder: str) -> dict:
    ...
    info = {
        "n_electrodes": n_elec,
        "electrode_positions": [
            {"id": i, "x": float(elec_pos[i, 0]), "y": float(elec_pos[i, 1])} for i in range(n_elec)
        ],
        "injection_pattern": {
            "type": "adjacent",
            "n_patterns": len(inj_pairs),
            "pairs": [{"source": int(src), "sink": int(sink)} for src, sink in inj_pairs],
        },
        "measurement_pattern": {
            "type": "adjacent_all",
...
def main():
    ...
    trunk = Trunk(
        ...
        anomalies=[
            ...
            Anomaly(shape=Circle(center=Pos(r=0.2, phi=np.pi), radius=0.1), conductivity=0.8),
        ],
    )
    ...
    n_elec = 16
    sim_params = {"grid_resolution": 64, "n_electrodes": n_elec, "injection_method": "adjacent"}
    ...
    print(
        "  Saved: input_trunk_config.json, input_simulation_params.json, input_electrode_config.json\n"
    )
    ...
    # Save EIDORS outputs
    save_voltages_readable(
        result.voltages, os.path.join(output_folder, "output_eidors_voltages.txt")
    )
    save_mesh_readable(result.mesh.nodes, result.mesh.elems, output_folder)
    save_conductivity_map(
        result.elem_data, os.path.join(output_folder, "output_elem_conductivity.txt")
    )
    ...
    print(
        "  Saved: output_eidors_*, output_mesh_*, output_elem_conductivity.*, *_visualization.png\n"
    )
    ...
    summary = {
        ...
        "files": {
            "inputs": [
                "input_trunk_config.json",
                "input_simulation_params.json",
                "input_electrode_config.json",
            ],
            "grid": [
                "output_conductivity_grid.npy",
                "output_conductivity_grid.png",
                "output_conductivity_grid.txt",
                "output_conductivity_visualization.png",
            ],
            "mesh": [
                "output_mesh_nodes.npy",
                "output_mesh_elems.npy",
                "output_mesh_nodes.csv",
                "output_mesh_elements.csv",
                "output_mesh_visualization.png",
            ],
            "eidors": [
                "output_eidors_voltages.npy",
                "output_eidors_voltages.txt",
                "output_elem_conductivity.npy",
                "output_elem_conductivity.txt",
            ],
