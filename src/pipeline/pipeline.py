...
def generate_random_trunk(config: PipelineConfig) -> Trunk:
    ...
    for _ in range(num_anomalies):
        anomaly_radius = np.random.uniform(
            config.anomaly_radius_range[0],
            config.anomaly_radius_range[1]
        )
        ...
        conductivity = np.random.uniform(
            config.anomaly_conductivity_range[0],
...
def save_dataset(
    ...
    if format == "hdf5":
        ...
        with h5py.File(output_path, 'w') as f:
            ...
            f.create_dataset('base_conductivities', data=[s.trunk_config.base_conductivity for s in samples])

            has_mesh = samples[0].mesh_voltages is not None
            if has_mesh:
                f.create_dataset('mesh_voltages', data=[s.mesh_voltages for s in samples if s.mesh_voltages is not None])
                f.create_dataset('mesh_elem_data', data=[s.mesh_elem_data for s in samples if s.mesh_elem_data is not None])
        ...
        has_mesh = samples[0].mesh_voltages is not None
        if has_mesh:
            data['mesh_voltages'] = [s.mesh_voltages.tolist() if s.mesh_voltages is not None else [] for s in samples]
            data['mesh_elem_data'] = [s.mesh_elem_data.tolist() if s.mesh_elem_data is not None else [] for s in samples]
            data['mesh_nodes'] = [s.mesh_nodes.tolist() if s.mesh_nodes is not None else [] for s in samples]
            data['mesh_elems'] = [s.mesh_elems.tolist() if s.mesh_elems is not None else [] for s in samples]
