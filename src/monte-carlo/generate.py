...
class RandomTreeFactory:
    ...
    def get_random_anomaly(
        self,
        harmonic_degree: int = 3,
        harmonic_sigma: float = 0.2,
        mu_r0_multipliyer: float = 0.15,
        sigma_r0: float = 0.05,
    ...
    def get_random_trunk(
        self, n_anomalies: int, harmonic_degree: int = 3, base_sigma: float = 0.2
    ) -> Trunk:
        if n_anomalies < 0:
            raise ValueError("El número de anomalías no puede ser negativo.")

        anomalies = [
            self.get_random_anomaly(harmonic_degree, base_sigma) for _ in range(n_anomalies)
        ]

        trunk = Trunk(
            radius=self.trunk_radius, base_conductivity=self.base_conductivity, anomalies=anomalies
        )
        return trunk
    ...
    def get_random_gaussian(self, mu: float = 0.0, sigma: float = 1.0, scale: float = 1.0) -> float:
